...
const ParallaxBackground = ({ frontUrl, backUrl, children }: Props) => {
  ...
  return (
    <div className={styles.parallax}>
      <img id="parallax-back" src={backUrl} alt="" className={styles.parallax__back} />
      <img id="parallax-front" src={frontUrl} alt="" className={styles.parallax__front} />
...
function handleScroll(this: HTMLElement, ev: Event) {
  ...
  if (background) {
    background.style.objectPosition = `0 ${scrollTop * -0.3}px`;
  }

  if (foreground) {
    foreground.style.objectPosition = `0 ${scrollTop * 0.1}px`;
  }
